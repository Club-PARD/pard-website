  ...
  const [Part, SetPart] = useState([ //part배열 생성 후 setPart 함수로 상태값을 업데이트
    {
      ...
      context:
        ...
        세미나는 전문가로부터 검토받은 커리큘럼을 기반으로 진행되며 파트장의 세션과 파트원들의 정보 공유, 현직자 실무 세션을 통해 
        개별 역량과 함께 협업을 위한 지식을 쌓습니다.<br />
        연합 세미나에서는 타 직군과 협업, 소통하는 방법을 배우고,<br />
    ...
    },
    {
      ...
      context:
        ...
        배우고 싶은 내용이 있다면 직군, 관심 분야와 관계없이<br /> 
        누구나 자유롭게 함께 주체적으로 학습할 사람을 모으고,<br />
    ...
    },
    {
      ...
      selected: false,
      context:
        <>11박 2일간 일상에서 벗어나 서로 더 깊게 함께 알아가고 즐기며,<br />
        다른 직군과 함께 일 할 때 가장 필요한 협업 커뮤니케이션 및 <br />
        방법론을 학습합니다.<br />
        조직 문화 및 커뮤니케이션 컨설팅사의 검수를 받은 자체 개발<br /> 
    ...
    },
    {
      ...
      selected: false,
      context:
        <>제시된 주제에 따라 무박 2일간 기획자, 디자이너, 개발자가 한 팀이 되어서
          MVP(Minimum Value Product) 수준의 서비스(웹/앱)를 빠르게 개발합니다.<br />
          제한된 리소스 내에서 서비스의 임팩트를 극대화 할 수 있는 방법을
    ...
    },
    {
      ...
      selected: false,
      context:
        <>파트와 무관하게 아이디어를 제시, 경선을 통해 팀을 구성합니다.<br />
        3주간 기획, 디자인, 개발 파트가 함께 배포 가능한 수준의<br />
        웹/앱 서비스를 만듭니다.<br />
        각 파트별 실무자들의 멘토링과 피드백을 통해 완성도를 높이고<br />데모데이를 통해 IT 협업 과정과 결과물에 대한 평가를 받습니다.</>
    },
    {
      id: 6,
      listNum: '06',
      ...
      context:
        ...
        ‘문제 해결 공동 프로젝트’ 형태로 서비스 개선 및 신규 아이디어를 발굴하며 실무 역량을 향상합니다.<br />
        아이디어 도출 과정, 시장이 필요로 하는 서비스를 만드는 데 있어 UX/UI 컨설팅펌과 협력을 통해 전문성을 확보하고 있습니다.</>
    },
  ]);

  const HandleButtonClick = (id) => { //현재 id
    SetPart(PrevPart => PrevPart.map(p => { //PrevPart 이전에 저장해둔 배열 part정보, p는 각 요소
      if (p.id === id) {
        return {
            ...
            <StyledWrap>
              {Part.map(content => (
                <StyledWrap2 key={content.id} >
                  <Button onClick={() => HandleButtonClick(content.id)}
              ...
              {Part.map(props => (
                <Body1 key={props.id} selected={props.selected}>{props.context}</Body1>
              ))}
            </StyledWrap>
          </ThemeProvider>
        </Part2Div>
